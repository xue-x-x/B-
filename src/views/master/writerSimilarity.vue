...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goTo (url,obj) {
        if(url.indexOf("writer") != -1){
          self.$cookies.set("writerSimilarityMId",obj.mid);
        }
        this.$router.push(
          {
            path: url,
      ...
      getWriter(){
        let self = this;
        self.$cookies.set("writerSimilarityParamsData",self.paramsData);
    ...
    mounted() {
      ...
      let tag = decodeURIComponent(self.$route.query.tag);
      let writerSimilarityMId = this.$cookies.get('writerSimilarityMId');
      let writerMId = this.$cookies.get('writerMId');
      let writerSimilarityParamsData = self.$cookies.get("writerSimilarityParamsData");
      if(writerSimilarityMId == writerMId && writerMId && writerSimilarityMId){
        // 判断是否是达人详情页返回
        self.paramsData = writerSimilarityParamsData;
        self.$cookies.set("writerSimilarityMId",0);
      }else {
        self.paramsData.tag = tag;
      }
